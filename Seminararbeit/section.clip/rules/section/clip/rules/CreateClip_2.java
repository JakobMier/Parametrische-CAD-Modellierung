...
public class CreateClip_2 extends JavaRule {
		...
		for (Panel p : panels) {
			for (int i = 0; i < section.getNStringer(); i++) {
				for (int j = 0; j < section.getNSpante(); j++) {
					...
					transforms.add(createClip_1(x, angle, height, helpComp));
		...
		for (Panel p : panels) {
			for (int i = 0; i < section.getNStringer(); i++) {
				for (int j = 0; j < section.getNSpante(); j++) {
					...
					floor.add(createClip_2(graph, transforms.get(counter), floorSkin));
					counter++;
				}
			}
		}
		...
		for (Panel p : panels) {
			for (int i = 0; i < section.getNStringer(); i++) {
				for (int j = 0; j < section.getNSpante(); j++) {
					...
					wall_1.add(createClip_3(x, angle, height, graph, transforms.get(counter), floorSkin, surface, helpG,
							u, l2, l4, hi, floor.get(counter)));
					counter++;
				}
			}
		}
...
//		createClip(500., 50., 3., floorSkin, cutSkin, surface);
	...
	private Transform createClip_1(double x, double angle, double height, Component helpComp) {
	...
	private TopoDS_Shape createClip_2(Graph<TopoDS_Shape> graph, Transform t, TopoDS_Shape floorSkin) {
	...
	private Block createClip_3(double x, double angle, double height, Graph<TopoDS_Shape> graph, Transform t,
			...
			TopoDS_Shape floor1) throws Exception {
...
//		helpG.getGeometric().add(point);
	...
	private void createClip_4(double x, double angle, Block wall_1, Graph<TopoDS_Shape> graph, TopoDS_Shape cutSkin,
			double u, double l2, double l4, double h, TopoDS_Face surface, TopoDS_Shape floor1) throws Exception {
