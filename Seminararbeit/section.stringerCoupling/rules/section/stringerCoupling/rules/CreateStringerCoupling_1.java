...
public class CreateStringerCoupling_1 extends JavaRule {
	...
	public void execute() throws Exception {
		...
		Component couplingComp = Component.create().setId("Stringer couplings");

		for (int i = 0; i < lofts.size(); i++) {
			for (int j = 0; j < transform.size(); j++) {
				Common com = Common.create();
				com.getDie().add(lofts.get(i));
				com.getDie().add(transform.get(j));

				StringerCoupling coupling = StringerCoupling.create().setId("Coupling (" + i + "," + j + ")");
				coupling.setShape(com);
				couplingComp.getSub().add(coupling);
				coupling.setZi(i);
				coupling.setXi(j);
				...
				coupling.getStringer().add(panel_1.getStringer().get(i % section.getNStringer()));
				coupling.getStringer().add(panel_2.getStringer().get(i % section.getNStringer()));
			}
		}

		couplingComp.setId("Stringer Couplings (" + couplingComp.getSub().size() + ")");
	}
