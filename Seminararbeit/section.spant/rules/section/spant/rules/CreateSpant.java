...
public class CreateSpant extends JavaRule {
	...
	public void execute() throws Exception {
		...
		SpantProfile spantProfile = SpantProfile.create();
		double stringerHeight = getGraph().firstInstance(StringerProfile.class).getHeight().getValue();
		spantProfile.setHeightForClip(t + r1 + l1 + stringerHeight);

		// Constructing profile for spant
...
//		Component comp_spant = Component.create().setId("spant profile");

		for (Geom_TrimmedCurve curve : segments) {
//			create43GeometryFromTopoDSShape(new BRepBuilderAPI_MakeEdge(curve).Edge(), "profile segment", comp_spant);
...
//		create43GeometryFromTopoDSShape(profileEdge, "profile", comp_spant);

		double dl = section.getL().getValue() / section.getNPanelX();

		Component spantsComp = Component.create().setId("Spante");

		// Creating Spants which are distributed at regular intervals defined by length
		// of section and number of panel columns

		for (Panel p : panels) {
			for (int i = 0; i < section.getNSpante(); i++) {

				Spant spant = Spant.create();
				spant.setId("Spant: " + i);
				spant.setI(i);
				p.getSpant().add(spant);
				...
				List<TopoDS_Face> spantSegments = createSpantSegments(guideLine, section.getSurface(), approxSegments,
						startParam, endParam);

				spantsComp.getSub().add(spant);

				for (TopoDS_Face segment : spantSegments) {

					Component spantSegmentComp = BRepApi.visualizeShape(segment).setId("Spant segment");
					spant.getSub().add(spantSegmentComp);
				}

			}
		}

		spantsComp.setId("Spante (" + spantsComp.getSub().size() + ")");

//////////////////////// FUNCTIONS //////////////////////////////////////////

	}

	// Most functions here written by supervisor Julian Dinkelacker

	private List<TopoDS_Face> createSpantSegments(Geom_Curve intersection, TopoDS_Face face,
			List<Geom_BSplineCurve> segments, double paramStart, double paramEnd) throws Exception {

		List<TopoDS_Face> spantSegments = new ArrayList<>();
		...
		for (int k = 0; k < segments.size(); k++) {
			...
			try {
				...
				spantSegments.add(gordonFace);

			} catch (Exception e) {
				e.printStackTrace();
				return spantSegments;
			}
		}
		return spantSegments;
	}
